...
function startFerrySimulation(marker, startPos, endPos, aisMarker = null) {
  ...
  function update() {
    ...
    if (aisOffsetMins >= 1 && aisOffsetMins < 5) {
      ...
      aisStatusMsg = `MoguÄ‡ prekid linije! (AIS)`
    }
    ...
    if (sidebarFerryRow) {
      if (aisOffsetMins >= 1) {
        ...
        sidebarFerryRow.innerHTML = `SljedeÄ‡i: <span style="color: #fff">${depStinica.next}</span>`
    ...
    if (sidebarLoparRow) {
      ...
      sidebarLoparRow.innerHTML = `SljedeÄ‡i: <span style="color: #fff">${loparNext}</span>`
    }
