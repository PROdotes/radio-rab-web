    ...
    try {
      if (state.ferryMarker && state.ferryMarker.getLatLng) {
        ...
        const distToMisnjak = Math.sqrt(
          Math.pow(currentLat - misnjak[0], 2) +
          Math.pow(currentLng - misnjak[1], 2)
        );
        const distToStinica = Math.sqrt(
          Math.pow(currentLat - stinica[0], 2) +
          Math.pow(currentLng - stinica[1], 2)
        );
      ...
      <div style="display: grid; gap: 1.5rem;">
        ...
        <div style="padding-top: 1rem; border-top: 1px solid var(--border);">
          ...
          <div style="font-size: 0.75rem; color: var(--text-dim); display: flex; justify-content: space-between; align-items: center;">
            ...
            <button onclick="window.open('https://www.vesselfinder.com/vessels/details/${data.imo}', '_blank')"
...
function initMap() {
  ...
  const ferryMarker = L.marker(misnjak, { icon: ferryIcon, pane: 'ferryPane', interactive: true })
    .addTo(state.layers.ferry)
